...
from confidence_chronograms.models import Cliente, Funcionario
...
def register_clie(request):
    if request.method == "POST":
        ...
        if form_usuario.is_valid():
            ...
            return redirect('account:register_cliente')
    else:
        form_usuario = UserCreationForm()
    return render(request, 'register_clie.html', {'form_usuario': form_usuario})

# mostra registro cliente para criar
@login_required(login_url="/login/")
...
def register_func(request):
    if request.method == "POST":
        ...
        if form_usuario.is_valid():
            form_usuario.save()
            return redirect('account:register_funcionario')
    else:
        form_usuario = UserCreationForm()
...
def submit_register_funcionario(request):
    ...
    return redirect("/confidencechonogra/funcionario")